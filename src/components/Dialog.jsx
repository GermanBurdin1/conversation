    ...
    try {
      ...
      if (response.ok) {
        ...
        const putResponse = await fetch(
          `http://localhost:5001/conversations/${id}`,
          {
            method: "PUT",
            headers: {
        ...
        if (putResponse.ok) {
          ...
          onSend(message, "user"); // Передача сообщения пользователя
          ...
          setTimeout(async () => {
            ...
            const updatedMessagesWithFakeResponse = [
              ...updatedMessages,
              fakeResponse,
            ];
            const updatedConversationWithFakeResponse = {
              ...updatedConversation,
              messages: updatedMessagesWithFakeResponse,
            };

            const putFakeResponse = await fetch(
              `http://localhost:5001/conversations/${id}`,
              {
                method: "PUT",
                headers: {
            ...
            if (putFakeResponse.ok) {
              ...
              onSend(fakeResponseContent, "assistant"); // Передача сообщения ассистента
            } else {
              console.error(
                "Failed to update conversation with fake response:",
                putFakeResponse.status
              );
            }
          }, 1000);
        } else {
        ...
        {localMessages.map((message, index) =>
          index % 2 === 0 && index + 1 < localMessages.length ? (
            <div key={index} className="message-pair">
              ...
              <button onClick={() => handleDeleteMessagePair(index)}>
                Delete
              </button>
            </div>
          ) : null
